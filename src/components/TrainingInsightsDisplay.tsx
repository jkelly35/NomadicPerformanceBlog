...
interface TrainingInsightsDisplayProps {
  showHeader?: boolean
  compact?: boolean
}

export default function TrainingInsightsDisplay({ showHeader = true, compact = false }: TrainingInsightsDisplayProps) {
  ...
  // Filter to only workout-related insights
  const trainingInsights = insights.filter(insight =>
    insight.type === 'workout' || insight.type === 'goals'
  )
      ...
      <div className={`${compact ? 'p-4' : 'p-6'} bg-white rounded-xl shadow-lg border border-gray-200`}>
        {showHeader && (
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-bold text-gray-800">Training Insights</h3>
          </div>
        )}
    ...
    return (
      <div className={`${compact ? 'p-4' : 'p-6'} bg-white rounded-xl shadow-lg border border-gray-200`}>
        {showHeader && (
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2">
              <div className="text-2xl">🏋️</div>
              <h3 className="text-xl font-bold text-gray-800">Training Insights</h3>
            </div>
            <button
        ...
        <div className="text-center py-8">
          ...
          <p className="text-gray-600 mb-2">No training insights available yet</p>
          <p className="text-sm text-gray-500">Start logging workouts and setting goals to get personalized training insights!</p>
        </div>
      </div>
    )
  }
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            <div className="text-sm text-gray-500">
              {trainingInsights.length} insight{trainingInsights.length !== 1 ? 's' : ''}
            </div>
            <button
              onClick={refreshInsights}
              disabled={loading}
