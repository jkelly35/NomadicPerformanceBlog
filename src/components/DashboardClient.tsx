          ...
          <div className="mb-12">
            <div className="flex items-center justify-between mb-8">
              ...
              <button
                ...
                className="bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white px-6 py-3 rounded-lg text-sm font-medium transition-all hover:scale-105 shadow-md hover:shadow-lg"
              >
                📈 View Full Analytics
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 shadow-lg border border-blue-200">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="text-right">
                    <div className="text-2xl font-bold text-blue-600">{data.weeklyStats.count}</div>
                    <div className="text-sm text-blue-600">this week</div>
                  </div>
                </div>
                <div className="text-sm font-medium text-blue-800">Workouts</div>
                <div className="text-xs text-blue-600 mt-1">Keep the momentum going!</div>
              </div>
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="text-right">
                    <div className="text-2xl font-bold text-orange-600">
                      {data.meals.length > 0 ? Math.round(data.meals.reduce((sum, meal) => sum + (meal.total_calories || 0), 0) / Math.max(data.meals.length, 1)) : 0}
                    </div>
                    <div className="text-sm text-orange-600">avg/day</div>
                  </div>
                </div>
                <div className="text-sm font-medium text-orange-800">Calories</div>
                <div className="text-xs text-orange-600 mt-1">Recent meal average</div>
              </div>
              ...
              <div className="bg-gradient-to-br from-cyan-50 to-blue-50 rounded-xl p-6 shadow-lg border border-cyan-200">
                <div className="flex items-center justify-between mb-4">
                  <div className="text-2xl">💧</div>
                  <div className="text-right">
                    <div className="text-2xl font-bold text-cyan-600">{Math.round(localHydrationTotal)}</div>
                    <div className="text-sm text-cyan-600">ml today</div>
                  </div>
                </div>
                <div className="text-sm font-medium text-cyan-800">Hydration</div>
                <div className="text-xs text-cyan-600 mt-1">
                  {localHydrationTotal >= 2500 ? 'Great job staying hydrated!' : 'Aim for 2500-3000ml daily'}
                </div>
              </div>
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="text-right">
                    <div className="text-2xl font-bold text-green-600">
                      {data.goals.length > 0 ? Math.round(data.goals.reduce((sum, goal) => sum + ((goal.current_value / goal.target_value) * 100), 0) / data.goals.length) : 0}%
                    </div>
                    <div className="text-sm text-green-600">avg progress</div>
                  </div>
                </div>
                <div className="text-sm font-medium text-green-800">Goals</div>
                <div className="text-xs text-green-600 mt-1">
                  {data.goals.filter(g => (g.current_value / g.target_value) >= 1).length} of {data.goals.length} completed
                </div>
              </div>
            </div>
          </div>
